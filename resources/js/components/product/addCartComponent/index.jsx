...
import { setCart, setTokenCart } from "../../../reducers/cartReducer";
...
export default function AddCartComponent({ dataBook }) {
    ...
    const renderPrice = () => {
        if (dataBook.book_price == dataBook.final_price) {
            return (
                <p className="price card-title" id="final-price">
                    ${dataBook.book_price}
                </p>
            );
        }
        ...
        return (
            <p className="price card-title">
                <span className="text-secondary">
                    <del>${dataBook.book_price}</del>
                </span>

                <span className="font-weight-bold ms-3" id="final-price">
                    ${dataBook.final_price}
                </span>
            </p>
        );
    };
    ...
    const handleAddToCart = () => {
        ...
        if (cart != null) {
            ...
            let bookInCart = cart.find((book) => book.id == dataBook.id);
            if (bookInCart) {
    ...
    return (
        <>
            <Card className="mb-3">
                <Card.Header>{renderPrice()}</Card.Header>
                <Card.Body>
