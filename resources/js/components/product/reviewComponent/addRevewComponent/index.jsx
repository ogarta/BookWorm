...
export default function AddReviewComponen({ dataBook }) {
    ...
    const onSubmit = (data) => {
        const postReview = async () => {
            try {
                const response = await ProductPageApi.addReview({
                    id: dataBook.id,
    ...
    return (
        <>
            <Card className="mb-3">
                <Card.Header>
                    <h5 className="title-add-review card-title">
                        Write a review
                    </h5>
                </Card.Header>
                <Card.Body>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="form-group">
                            ...
                            {errors.reviewTitle?.type === "maxLength" && (
                                <p role="alert" style={{ color: "red" }}>
                                    Review title max length 120 characters
                                </p>
                            )}
                            <p className="mt-2">
                            ...
                            <button
                                type="submit"
                                className="btn-sumit-add-review btn btn-secondary mt-3"
                            >
                                Submit Review
                            </button>
                        </div>
